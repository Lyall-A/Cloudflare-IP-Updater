...
(async function check() {
    await fetch(config.ipEndpoint).then(i => i.json()).then(async ipJson => {
        ...
        console.log(`IP changed from '${lastIp || "nothing"}' to '${ip || "nothing"}'`);
        ...
        await fetch(`https://api.cloudflare.com/client/v4/zones/${config.zoneId}/dns_records/${config.dnsRecordId}`, {
            ...
            if (res.status === 200) {
                lastIp = ip;
                return console.log("Succesfully updated DNS!");
            }

            const json = await res.json().catch(err => { });
            return console.log(`Failed to update DNS! Status: ${res.status}, JSON:`, json);
        }).catch(err => console.log("Failed to update DNS! Error:", err));
    }).catch(err => console.log("Failed to get IP! Error:", err));
